...
async fn sync_plugins_from_remote_reconciles_remote_cache_and_config() {
    ...
    write_plugin(
        &tmp.path().join("plugins/cache/chatgpt-workspace"),
        "orphan/local",
        "orphan",
    );
    write_file(
  ...
  "pagination": {
    ...
    assert_eq!(
        ...
        RemotePluginSyncResult {
            ...
            upgraded_plugin_ids: Vec::new(),
            enabled_plugin_ids: Vec::new(),
            disabled_plugin_ids: vec!["linear@chatgpt-global".to_string()],
            uninstalled_plugin_ids: vec![
                "orphan@chatgpt-workspace".to_string(),
                "stale@chatgpt-global".to_string(),
            ],
        }
    );
    assert!(
    ...
    assert!(
        !tmp.path()
            .join("plugins/cache/chatgpt-global/stale")
            .exists()
    );
    assert!(
        !tmp.path()
            .join("plugins/cache/chatgpt-workspace/orphan")
            .exists()
    );
    ...
    assert!(!config.contains(r#"[plugins."stale@chatgpt-global"]"#));
}
...
"#,
    ...
    assert!(
        tmp.path()
            .join("plugins/cache/chatgpt-global/stale/local")
            .is_dir()
    );
    let config = fs::read_to_string(tmp.path().join(CONFIG_TOML_FILE)).unwrap();
    assert!(config.contains(r#"[plugins."stale@chatgpt-global"]"#));
...
"#,
    ...
    assert!(matches!(
        err,
        PluginRemoteSyncError::UnexpectedStatus { .. }
    ));
    assert!(
        tmp.path()
            .join("plugins/cache/chatgpt-global/stale/local")
