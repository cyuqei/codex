...
- `devflowReleasePrep/submit` — Phase 8 Devflow publish/submit endpoint; keep `devflowReleasePrep/create` as the read-only gate, then queue a `release_publish` approval for Git mutation. `commit_only` runs `git add -A -- . ':(exclude).codex'` and `git commit -F <commit_message_artifact>` after approval; `commit_and_push` additionally requires an `origin` remote plus a current branch and runs `git push origin <currentBranch>` after the commit. Submit writes a persisted `Release publish report` artifact with command output and emits `devflowArtifact/created`; PR creation remains out of scope, and the PR-body artifact stays the handoff material for external review tools.
...
- `devflowArtifact/deliver` — Phase 6 artifact delivery endpoint; `codex-main` now handles local handoff receipts for the Codex main lane, while `hermes-automation` remains the legacy adapter for external destinations. `codex-main` accepts only `local` / `local:*` destinations and writes a receipt artifact without sending an external message. External destinations such as chat or webhook targets still return `status: "pending_approval"` plus a `devflowApproval/requested` item of kind `artifact_delivery`; Hermes is only invoked after `devflowApproval/respond` accepts that approval, and the final `delivery_receipt` is emitted later as `devflowArtifact/created`. This remains separate from `devflowArtifact/export`, which is still the pure file-copy API and the main-path handoff for Warp.
