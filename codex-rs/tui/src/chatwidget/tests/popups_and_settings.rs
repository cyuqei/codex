...
use codex_app_server_protocol::ProviderAuthStyle as AppServerProviderAuthStyle;
use codex_app_server_protocol::ProviderWireApi as AppServerProviderWireApi;
...
#[tokio::test]
async fn settings_popup_includes_default_provider_entry_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    let current_provider_id = chat.config.model_provider_id.clone();
    chat.open_settings_popup(&current_provider_id);

    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert_chatwidget_snapshot!("settings_popup_with_provider_entry", popup);
}

#[tokio::test]
async fn provider_preferences_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.model_provider_id = "deepseek".to_string();
    let mut deepseek =
        create_oss_provider_with_base_url("https://api.deepseek.com/v1", WireApi::ChatCompletions);
    deepseek.name = "DeepSeek".to_string();
    let mut openrouter =
        create_oss_provider_with_base_url("https://openrouter.ai/api/v1", WireApi::Responses);
    openrouter.name = "OpenRouter".to_string();
    chat.config
        .model_providers
        .insert("deepseek".to_string(), deepseek);
    chat.config
    ...
    let current_provider_id = chat.config.model_provider_id.clone();
    let providers = chat.config.model_providers.clone();
    chat.open_provider_preferences_popup(&current_provider_id, &providers);

    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert_chatwidget_snapshot!("provider_preferences_popup", popup);
}

#[tokio::test]
async fn provider_preferences_popup_emits_open_detail_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.model_provider_id = "deepseek".to_string();
    let mut deepseek =
        create_oss_provider_with_base_url("https://api.deepseek.com/v1", WireApi::ChatCompletions);
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::OpenProviderDetailPopup { provider_id })
            if provider_id == "openrouter"
    );
}

#[tokio::test]
async fn provider_preferences_popup_emits_open_create_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.config.model_provider_id = "deepseek".to_string();
...
fn type_into_bottom_popup(chat: &mut ChatWidget, text: &str) {
...
#[tokio::test]
async fn provider_create_form_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.open_provider_create_prompt();

    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert_chatwidget_snapshot!("provider_create_form_popup", popup);
}

#[tokio::test]
async fn provider_create_form_emits_submit_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;

    chat.open_provider_create_prompt();
    type_into_bottom_popup(&mut chat, "OpenRouter Custom");
    chat.handle_key_event(KeyEvent::from(KeyCode::Tab));
    type_into_bottom_popup(&mut chat, "https://openrouter.ai/api/v1");
    chat.handle_key_event(KeyEvent::from(KeyCode::Tab));
    type_into_bottom_popup(&mut chat, "sk-test-openrouter");
    chat.handle_key_event(KeyEvent::new(KeyCode::Char('s'), KeyModifiers::CONTROL));

    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::SubmitProviderCreatePrompt { params })
            if params.id == "openrouter-custom"
                && params.provider.display_name == "OpenRouter Custom"
                && params.provider.base_url == "https://openrouter.ai/api/v1"
                && params.provider.wire_api == AppServerProviderWireApi::Responses
                && params.provider.auth_style == AppServerProviderAuthStyle::Bearer
                && params.provider.env_key.is_none()
                && params.provider.api_key.as_deref() == Some("sk-test-openrouter")
                && params.provider.request_max_retries == Some(4)
                && params.provider.stream_max_retries == Some(5)
                && params.provider.stream_idle_timeout_ms == Some(300000)
    );
}

#[tokio::test]
async fn provider_edit_form_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    let mut openrouter =
        create_oss_provider_with_base_url("https://openrouter.ai/api/v1", WireApi::Responses);
    openrouter.name = "OpenRouter".to_string();
    openrouter.env_key = Some("OPENROUTER_API_KEY".to_string());
    openrouter.request_max_retries = Some(4);
    openrouter.stream_max_retries = Some(5);
    openrouter.stream_idle_timeout_ms = Some(300000);

    chat.open_provider_edit_prompt("openrouter", &openrouter);

    let popup = render_bottom_popup(&chat, /*width*/ 100);
    assert_chatwidget_snapshot!("provider_edit_form_popup", popup);
}

#[tokio::test]
async fn provider_edit_form_emits_submit_event() {
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::SubmitProviderEditPrompt { params })
            if params.id == "openrouter"
                && params.provider.display_name == "OpenRouter"
                && params.provider.base_url == "https://openrouter.ai/api/v1"
                && params.provider.wire_api == AppServerProviderWireApi::Responses
                && params.provider.auth_style == AppServerProviderAuthStyle::Bearer
                && params.provider.env_key.as_deref() == Some("OPENROUTER_API_KEY")
                && params.provider.api_key.is_none()
                && params.provider.request_max_retries == Some(4)
                && params.provider.stream_max_retries == Some(5)
                && params.provider.stream_idle_timeout_ms == Some(300000)
    );
...
#[tokio::test]
async fn provider_detail_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    chat.config.model_provider_id = "deepseek".to_string();
    let mut openrouter =
        create_oss_provider_with_base_url("https://openrouter.ai/api/v1", WireApi::Responses);
    ...
    chat.open_provider_detail_popup("openrouter", &openrouter, "deepseek", "gpt-5.4");

    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert_chatwidget_snapshot!("provider_detail_popup", popup);
}

#[tokio::test]
async fn provider_detail_popup_emits_open_models_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    chat.config.model_provider_id = "deepseek".to_string();
    let mut openrouter =
        create_oss_provider_with_base_url("https://openrouter.ai/api/v1", WireApi::Responses);
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::OpenProviderModelsPopup { provider_id }) if provider_id == "openrouter"
    );
}

#[tokio::test]
async fn provider_detail_popup_emits_test_connection_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    chat.config.model_provider_id = "deepseek".to_string();
    let mut openrouter =
        create_oss_provider_with_base_url("https://openrouter.ai/api/v1", WireApi::Responses);
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::TestProviderConnection { provider_id }) if provider_id == "openrouter"
    );
}

#[tokio::test]
async fn provider_detail_popup_emits_open_edit_prompt_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    chat.config.model_provider_id = "deepseek".to_string();
    let mut openrouter =
        create_oss_provider_with_base_url("https://openrouter.ai/api/v1", WireApi::Responses);
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::OpenProviderEditPrompt { provider_id }) if provider_id == "openrouter"
    );
}

#[tokio::test]
async fn provider_detail_popup_emits_delete_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    chat.config.model_provider_id = "deepseek".to_string();
    let mut openrouter =
        create_oss_provider_with_base_url("https://openrouter.ai/api/v1", WireApi::Responses);
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::OpenProviderDeleteConfirm { provider_id }) if provider_id == "openrouter"
    );
}

#[tokio::test]
async fn provider_model_catalog_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("cap-model")).await;
    let preset = ModelPreset {
        id: "cap-model".to_string(),
        model: "cap-model".to_string(),
        display_name: "Capability Model".to_string(),
        description: "Capability-rich model".to_string(),
    ...
    chat.open_provider_model_catalog_popup("openrouter", vec![preset]);
    let popup = render_bottom_popup(&chat, /*width*/ 80);

    assert!(popup.contains("Models for openrouter"));
    assert!(popup.contains("Default"));
    assert!(popup.contains("Capabilities: vision, personality, fast, reasoning"));
    assert_chatwidget_snapshot!("provider_model_catalog_popup", popup);
}

#[tokio::test]
async fn provider_model_catalog_popup_emits_persist_provider_model_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("cap-model")).await;
    let preset = ModelPreset {
        id: "cap-model".to_string(),
        model: "cap-model".to_string(),
        display_name: "Capability Model".to_string(),
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::PersistProviderModelPreferenceSelection {
            provider_id,
            model,
            effort,
            service_tier,
...
#[tokio::test]
async fn provider_model_catalog_popup_emits_open_provider_service_tier_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("cap-model")).await;
    let preset = ModelPreset {
        id: "cap-model".to_string(),
        model: "cap-model".to_string(),
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::OpenProviderServiceTierPopup { provider_id, model, effort })
            if provider_id == "openrouter"
                && model == "cap-model"
                && effort == Some(ReasoningEffortConfig::Medium)
    );
...
#[tokio::test]
async fn provider_model_catalog_popup_emits_open_provider_reasoning_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("cap-model")).await;
    let preset = ModelPreset {
        id: "cap-model".to_string(),
        model: "cap-model".to_string(),
    ...
    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::OpenProviderReasoningPopup { provider_id, model })
            if provider_id == "openrouter" && model.model == "cap-model"
    );
}

#[tokio::test]
async fn provider_reasoning_popup_emits_persist_provider_model_event_with_effort() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("cap-model")).await;
    let preset = ModelPreset {
        id: "cap-model".to_string(),
        model: "cap-model".to_string(),
    ...
    chat.open_provider_reasoning_popup("openrouter", preset);
    chat.handle_key_event(KeyEvent::from(KeyCode::Down));
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
...
#[tokio::test]
async fn provider_reasoning_popup_emits_open_provider_service_tier_event_with_effort() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("cap-model")).await;
    let preset = ModelPreset {
        id: "cap-model".to_string(),
        model: "cap-model".to_string(),
...
#[tokio::test]
async fn provider_service_tier_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("cap-model")).await;
    chat.open_provider_service_tier_popup(
        "openrouter",
        "cap-model",
    ...
    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert_chatwidget_snapshot!("provider_service_tier_popup", popup);
}

#[tokio::test]
async fn provider_service_tier_popup_emits_persist_provider_model_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("cap-model")).await;
    chat.open_provider_service_tier_popup(
        "openrouter",
        "cap-model",
...
#[tokio::test]
async fn commit_workflow_prompt_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.open_commit_workflow_prompt(
        "Draft a git commit message.\n\n## Git status\n```text\n## main\n M src/lib.rs\n```"
            .to_string(),
    );

    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert_chatwidget_snapshot!("commit_workflow_prompt", popup);
}

#[tokio::test]
async fn commit_workflow_prompt_emits_submit_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.open_commit_workflow_prompt("Draft a git commit message.".to_string());
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));

    assert_matches!(
        rx.try_recv(),
        Ok(AppEvent::SubmitCommitWorkflowPrompt { draft }) if draft == "Draft a git commit message."
    );
}

#[tokio::test]
async fn context_workflow_popup_snapshot() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.open_context_workflow_popup();

    let popup = render_bottom_popup(&chat, /*width*/ 80);
    assert_chatwidget_snapshot!("context_workflow_popup", popup);
}

#[tokio::test]
async fn context_workflow_popup_emits_save_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.open_context_workflow_popup();
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
...
#[tokio::test]
async fn context_workflow_popup_emits_restore_event() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    chat.open_context_workflow_popup();
    chat.handle_key_event(KeyEvent::from(KeyCode::Down));
    chat.handle_key_event(KeyEvent::from(KeyCode::Enter));
...
#[tokio::test]
async fn context_save_summary_snapshot() {
    let (mut chat, mut rx, _op_rx) = make_chatwidget_manual(Some("gpt-5.4")).await;
    chat.thread_id = Some(ThreadId::new());
    chat.thread_name = Some("my-thread".to_string());
    chat.current_cwd = Some(std::path::PathBuf::from("/tmp/project"));
    chat.current_rollout_path = Some(std::path::PathBuf::from("/tmp/rollout.jsonl"));

    chat.show_context_save_summary();
    ...
    let rendered = cells
        ...
        .collect::<Vec<_>>()
        .join("\n");
    insta::assert_snapshot!("context_save_summary", rendered);
}
...
#[tokio::test]
async fn model_picker_selected_description_shows_capabilities() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(Some("cap-model")).await;
    let preset = ModelPreset {
        id: "cap-model".to_string(),
        model: "cap-model".to_string(),
    ...
    assert!(popup.contains("Default"));
    assert!(popup.contains("Capabilities: vision, personality, fast, reasoning"));
    assert!(popup.contains("Reasoning levels: low, high"));
    assert!(popup.contains("Service tiers: Fast"));
}
