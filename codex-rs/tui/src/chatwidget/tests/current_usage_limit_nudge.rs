...
fn explicit_inactive_snapshot() -> RateLimitSnapshot {
    RateLimitSnapshot {
...
fn missing_nudge_snapshot() -> RateLimitSnapshot {
    snapshot(/*percent*/ 75.0)
}
...
async fn proactive_usage_prompt_explicit_null_clears_suppression() {
    ...
    chat.on_rate_limit_snapshot(Some(explicit_inactive_snapshot()));
    chat.on_rate_limit_snapshot(Some(snapshot_with_nudge(
        "repeatable-key",
        /*threshold*/ 75,
        UsageLimitNudgeCopyVariant::AddCredits,
...
#[tokio::test]
async fn proactive_usage_prompt_missing_field_preserves_suppression() {
    let (mut chat, _rx, _op_rx) = make_chatwidget_manual(/*model_override*/ None).await;
    ...
    chat.on_rate_limit_snapshot(Some(missing_nudge_snapshot()));
    chat.on_rate_limit_snapshot(Some(snapshot_with_nudge(
        "sticky-key",
        /*threshold*/ 75,
        UsageLimitNudgeCopyVariant::AddCredits,
...
async fn proactive_usage_prompt_waits_for_between_turn_hook() {
    ...
    assert!(render_bottom_popup(&chat, /*width*/ 88).is_empty());
