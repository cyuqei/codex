...
fn render_footer_lines(
    ...
    let mut parts = match sort_key {
        ThreadSortKey::CreatedAt => vec![
            FooterPart::Text(created.to_string()),
            FooterPart::Text(format!("updated {updated}")),
        ],
        ThreadSortKey::UpdatedAt => vec![
            FooterPart::Text(updated.to_string()),
            FooterPart::Text(format!("created {created}")),
        ],
    };
    if let Some(branch) = branch {
        parts.push(FooterPart::Text(branch.to_string()));
    } else {
        parts.push(FooterPart::NoBranch);
    }
    if let Some(cwd) = cwd {
        parts.push(FooterPart::Text(cwd.to_string()));
    }
...
impl FooterPart {
    fn text(&self) -> &str {
        match self {
            FooterPart::Text(text) => text,
            FooterPart::NoBranch => "no branch",
        }
    }

    fn truncate(self, width: usize) -> Self {
        match self {
            FooterPart::Text(text) => FooterPart::Text(truncate_text(&text, width)),
            FooterPart::NoBranch => FooterPart::NoBranch,
        }
    }
}
...
fn pack_footer_parts(parts: Vec<FooterPart>, width: u16) -> Vec<Line<'static>> {
    ...
    for part in parts {
        let separator_width = if current.is_empty() { 0 } else { 5 };
        let part_width = part.text().chars().count();
        if !current.is_empty() && current_width + separator_width + part_width > width as usize {
            lines.push(footer_line(current));
            current = Vec::new();
            current_width = 2;
        }
        let available_width = width as usize - current_width;
        let part = part.truncate(available_width);
        current_width += separator_width + part.text().chars().count();
        current.push(part);
    }
    if !current.is_empty() {
        lines.push(footer_line(current));
...
fn footer_line(parts: Vec<FooterPart>) -> Line<'static> {
    ...
    for (idx, part) in parts.into_iter().enumerate() {
        ...
        match part {
            FooterPart::Text(part) => spans.push(part.dim()),
            FooterPart::NoBranch => spans.push("no branch".dim().italic()),
        }
    }
    spans.into()
}
