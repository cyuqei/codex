...
impl AnalyticsReducer {
    ...
    fn ingest_subagent_thread_started(
        ...
        if let Some(parent_connection_id) = parent_thread_id
            .as_ref()
            .and_then(|parent_thread_id| self.thread_connections.get(parent_thread_id))
        {
            self.thread_connections
                .insert(input.thread_id.clone(), *parent_connection_id);
        }
    ...
    fn ingest_notification(
        ...
        match notification {
            ...
            ServerNotification::ItemCompleted(notification) => {
                ...
                let Some(connection_id) = self.thread_connections.get(&notification.thread_id)
                else {
                    tracing::warn!(
                        thread_id = %notification.thread_id,
                        turn_id = %notification.turn_id,
                        item_id,
                        "dropping tool item analytics event: missing thread connection metadata"
                    );
                    return;
                };
                let Some(connection_state) = self.connections.get(connection_id) else {
                    tracing::warn!(
                        ...
                        item_id,
                        connection_id,
                        "dropping tool item analytics event: missing connection metadata"
                    );
                    return;
                };
                let Some(thread_metadata) = self.thread_metadata.get(&notification.thread_id)
                else {
                    tracing::warn!(
                        thread_id = %notification.thread_id,
                        turn_id = %notification.turn_id,
                        item_id,
                        "dropping tool item analytics event: missing thread lifecycle metadata"
                    );
                    return;
                };
    ...
    fn emit_tool_review_event_at(
        ...
        let Some(thread_metadata) = self.thread_metadata.get(&pending_review.thread_id) else {
            tracing::warn!(
        ...
        ThreadItem::CommandExecution {
            ...
            let base = tool_item_base(
                ...
                ToolItemOutcome {
                    terminal_status,
                    failure_kind,
                    duration_ms: item_or_observed_duration_ms(
        ...
        ThreadItem::FileChange {
            ...
            let base = tool_item_base(
                ...
                ToolItemOutcome {
                    terminal_status,
                    failure_kind,
                    duration_ms: observed_duration_ms(started_at, completed_at),
                },
                ToolItemContext {
                    started_at,
                    completed_at,
...
fn item_or_observed_duration_ms(
...
fn observed_duration_ms(started_at: u64, completed_at: u64) -> Option<u64> {
    item_or_observed_duration_ms(/*item_duration_ms*/ None, started_at, completed_at)
}
